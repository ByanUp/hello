...
его конструкторе инициализировать параметр, соответствующий количеству ячеек клетки (целое число). В классе должны быть
...
class Unit:
    ...
    def __add__(self, other):
        return f'Объединение двух клеток: {self.value + other.value}'

    def __sub__(self, other):
        sub = self.value - other.value
        return f'Вычитание клеток: {sub}' if sub > 0 else 'Разность меньше нуля, повторите запрос, но с положительным ' \
                                                          'результатом '
